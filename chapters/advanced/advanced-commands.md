...
These commands allow one to perform steps with bigger granularity than the basic ones, as it may be tedious to debug a program with basic steps.
Some of these commands are experimental features to skip the execution of some parts of the code under debug.

The debugging commands offered by the StDebugger in the "Advanced step" toolbar are described in the following sections.

### **Next instance creation:** 
Steps the execution until a class is instanciated in the current context.
...
Steps the execution until a message is sent to the current context's receiver.
...
### **Next call in class:** 
Steps the execution until a message is sent to any instance of the class of the current context's receiver.
...
### **To return:** 
Steps the execution until the current context is about to return, whether this is via a normal return, a non-local return or an implicit return, or until an unhandled exception is raised.
...
### **To method entry:** 
Steps the execution until a method is called, to stop at the start of its execution.
...
### **Skip:** 
Skips an instruction without executing it. 
...
### **Skip up to:** 
Skips several instructions in the same context, without executing them, up to the instruction under the caret.
...
Jumps to the instruction under caret that can be anywhere in the home context, without changing the state of the program.
...
![After jumping to caret.%width=70&label=fig:after-jump](graphics/after-jump-to-caret.png)
